(function () {
    ...
    const PENDING_CLOUD_CHANGES_KEY = 'tree-map-pending-cloud-changes-v1';
    ...
    async function ensureCloudSpace(space) {
        ...
        const rows = await supabaseRequest(`tree_map_spaces?id=eq.${encodeURIComponent(normalized.id)}&select=id`);
        if (Array.isArray(rows) && rows.length) {
            await supabaseRequest(`tree_map_spaces?id=eq.${encodeURIComponent(normalized.id)}`, {
                method: 'PATCH',
                body: JSON.stringify({
    ...
    async function deleteCloudSpace(spaceId) {
        await supabaseRequest(`tree_map_states?space_id=eq.${encodeURIComponent(spaceId)}`, {
            method: 'DELETE',
            headers: { Prefer: 'return=representation' },
        });
        const deletedSpaces = await supabaseRequest(`tree_map_spaces?id=eq.${encodeURIComponent(spaceId)}&select=id`, {
            method: 'DELETE',
            headers: { Prefer: 'return=representation' },
        });
        if (!Array.isArray(deletedSpaces) || !deletedSpaces.length) {
            const stillExists = await supabaseRequest(`tree_map_spaces?id=eq.${encodeURIComponent(spaceId)}&select=id`);
    ...
    const TreeMapStorage = {
        ...
        async getCloudState(space) {
            ...
            try {
                ...
                const cloudState = snapshot && snapshot.content ? normalizeState(snapshot.content) : createSampleState();
    ...
    window.addEventListener('online', () => {
        ...
        TreeMapStorage.flushPendingCloudChanges().catch(error => {
            console.error('Tree Map cloud sync retry failed:', error);
            if (spaceStatus) spaceStatus.textContent = `Cloud sync pending: ${formatErrorMessage(error)}`;
        });
    });
    ...
    setTimeout(() => {
        ...
        TreeMapStorage.flushPendingCloudChanges().catch(error => {
            console.error('Tree Map cloud sync startup retry failed:', error);
            if (spaceStatus) spaceStatus.textContent = `Cloud sync pending: ${formatErrorMessage(error)}`;
        });
    }, 0);
